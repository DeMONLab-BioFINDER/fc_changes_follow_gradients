plot_mediation_gradients <- function(subject_data, treatments, outcomes, covariates, gradient_data, gradients = c(1, 3),
  ...
  for(i in seq_along(treatments)) {
    ...
    brain_plot <- plot_ests |> 
      ...
      ggplot() +
      geom_sf(aes(geometry = geometry, fill = indirect), show.legend = FALSE) +
      (if (add_shade)
    ...
    med_plot <- ggdraw() +
      draw_label(paste0("n = ", n), x = 0.05, y = 0.975, hjust = 0) +
  ...
  for (g in grad_char) {
    for (i in seq_along(treatments)) {
      ...
      lab_term <- paste0("Indirect ", terms_nice[i], "→", outcomes[i]) 
...
plot_mediation_structure <- function(treat, mediator, outcome, padding = 0.4) {
  
  treat <- dplyr::case_when(
    treat == "pathology_ad" ~ "AD Path",
    treat == "age" ~ "Age",
    TRUE ~ treat
  )
