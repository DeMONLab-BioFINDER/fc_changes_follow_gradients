...
In the BioFINDER-2 cohort, the spatial distribution of node-wise associations between FCS and AD pathology (i.e. @fig-concept D) aligned with Gradient 1 (r = 0.74, p~spin~\<0.001), reflecting decreases of FCS in sensory-motor (unimodal) regions and increases in associative (transmodal) regions with increasing pathology. Age effects were instead independently aligned with Gradient 3 (r = 0.75, p~spin~\<0.001), with executive areas showing increasing and non-executive areas decreasing FCS with increasing age (@fig-fig1 A, @suppfig-mainoverlaid). Notably, age effects were not strongly aligned with Gradient 1, and AD effects were not strongly aligned with Gradient 3, suggesting a double dissociation. All variance inflation factors were below 1.2, indicating no issue of multicollinearity between age and pathology. These findings were replicated using a subset of the ADNI cohort (129 participants with complete data), see @fig-fig1 B (Gradient 1 for pathology: r = 0.54, p~spin~\<0.001; Gradient 3 for age: r = 0.68, p~spin~\<0.001). Due to a non-existant or modest and non-reproducible relationship between the t-maps and Gradient 2 (see @suppfig-grad2), this gradient was left out of the present and subsequent analyses for brevity. Separate analyses of within- and between-network affinity (@suppfig-within, @suppfig-between) revealed similar patterns to overall affinity, with generally weaker gradient relationships for within- compared to between-network affinity. This suggests that both contribute to the observed gradient alignments, but between-network connections play a greater role. Interestingly, using nodal connectivity strength (the average connection weight of each parcel and more conventional FC measure), we obtained very similar spatial distributions of effects (see @suppfig-strength) even though the measure implies a different interpretation. We also ran the analysis with tau and Aβ separated as well as with APOE ε4 status as a predictor, the results presented in @suppfig-tauAb indicated that tau, more so than Aβ positivity, seems to be driving the alignment and that APOE ε4 carriership was not associated with gradient aligned effects. Finally, in an additional model including cortical thickness, white-matter hyperintensity burden, lacunes, microbleeds, and CSF α-synuclein positivity (@suppfig-cerebvasc), the characteristic gradient alignments for age (Gradient 3) and AD pathology (Gradient 1) remained largely unchanged, indicating that these effects are not explained by atrophy or cerebrovascular confounds. Interestingly, several of the additional covariates also  showed independent effects aligning with the gradients.
...
Prior studies on FC in AD have reported an initial pattern of regional hyperconnectivity, which is subsequently followed by connectivity declines [@corriveau-lecavalier2024; @schultz2017; @badhwar2017]. Nonlinear relationships between age and FC have also been reported, e.g. [@ng2016; @sun2025]. We therefore sought to investigate whether these nonlinearities were reflected similarly in gradient-like FCS changes. To investigate these potential nonlinearities, generalized additive models (GAMs) were fitted with FCS at each parcel as the outcome, and age, pathology and relevant covariates as predictors. The derivatives of these models capture the direction and strength (i.e., slope) of each predictor’s effect across its range. For each predictor (age and pathology), the estimated slopes result in cortical “slope-maps” that reflect the regional strength and direction of the predictor’s effect, at any value of the predictor. This allows us to observe increases or decreases at specific points across the age and pathology spectra. For pathology, the relationship between Gradient 1 and the slope-maps increased up to a pathology level of \~0.25 (representing early AD pathology), remaining relatively high until \~0.5 (representing middle-stages of AD pathology), after which it declined sharply with further accumulation (@fig-gam B.2). For age, the relationship between Gradient 3 and the slope-maps remained high between ages 55 and 70, but essentially vanished after age 80. These results suggest that gradient-like FCS patterns emerge as an initial response to pathological or physiological changes but diminish as these changes progress.
...
::: {#fig-agepath}
![](revision_figures/age_path_dist.png)

Distribution of age and pathology score of the main cross-sectional sample. The scatter plot indicates a non-linear relationship between the two leaving a lot of independent variation. 
:::
...
::: {#suppfig-neuroquery .column-body-outset}
![](revision_figures/gradient_coverage.png)

Top: Spatial distribution of gradient scores for Gradient 1 (left) and Gradient (3) respectively, derived from biofinder. 
Middle: Meta-analytic decoding was performed using the NeuroQuery database [@dockès2020] to identify cognitive terms associated with each Schaefer-1000 parcel. For each parcel, regional associations with NeuroQuery terms were estimated using the ROIAssociationDecoder in NiMARE [@Salo2023], and results were aggregated across parcels. The resulting term associations were filtered to include only cognitive concepts listed in the Cognitive Atlas [@poldrack2011], matched by approximate string similarity, and visualized as a word cloud highlighting their spatial alignment along the principal functional gradient.
Bottom: Distribution of gradient scores across the 7 Yeo networks.
...
::: {#suppfig-tauAb .column-body-outset}

![](revision_figures/apoe_plot_v1_w_tau_ab.png)

\A A\) Main results with separated pathology in the full sample controlling for diagnosis due to all MCI/AD being Aβ+ by definition in this study. Tau pathology is here calculated as a non-linear mapping of tau PET SUVR from Braak12, Braak34, Braak56 in the same manner as for the main pathology composite. Aβ positivity was determined using previously established amyloid-PET cut-offs derived in [@biel2025]. B) Main results with separated pathology in cognitively unimpaired. Tau seems to be the main driver of the alignment to the sensory-association axis. C) Main result with APOE ε4 carriership in the model in the full sample. D) Since the relationship between being an ε4 carrier and being Aβ+ is strong, and being Aβ+ is strongly related to being MCI/AD we also ran the analysis is cognitively unimpaired Aβ- individuals where we can see that being an ε4 carrier in itself does not give rise to gradient alignment. 
:::

::: {#suppfig-cerebvasc .column-body-outset}

![](revision_figures/atrophy_and_cerebrovasc.png)

The FCS patterns associated with age and AD remained stable after including atrophy and several vascular markers as covariates in the main analysis. The parcelwise model included age, tau pathology, cortical thickness in the AD atrophy signature region, white-matter hyperintensity burden, presence of lacunes, presence of microbleeds, and CSF α-synuclein positivity (RTQuIC). Scatterplots show the spatial correlation between each effect map and the sensory–association (Gradient 1) and representational–executive (Gradient 3) axes. The principal age- and tau-related gradient alignments reported in the main manuscript remain robust after accounting for these factors, which themselves seemingly give rise to gradient aligned effects. 
...
![](revision_figures/all_grad_plot.png)

To investigate whether the lost association between AD pathology and the sensory association axis in the cognitively impaired subgroup analysis was related to a bias in deriving the gradients only from cognitively healthy subjects below the age of 60, we derived gradients from the full sample and ran the same analysis with consistent results. 

:::

::: {#suppfig-switchgrad .column-body-outset}
![](revision_figures/switched_grad_all_path.png)

Main results but using gradients derived from ADNI to interpret the t-maps in BioFINDER (A) and the gradients derived from BioFINDER to interpret the t-maps from ADNI (B). To investigate whether the calculation pathology scores within each cohort affected the results we derived a pathology score after pooling the two samples and running the main analysis again in both BioFINDER (C) and ADNI (D). Results were comparable with the originally derived pathology score within each cohort. 

:::

::: {#suppfig-nodaltau .column-body-outset}
![](revision_figures/nodal_tau.png)
Results from parcelwise models using tau PET SUVR extracted for each Schaefer-1000 parcel (FCS~parcel~ /~ age + tau~parcel~ + covariates). The spatial pattern closely matches analyses with global pathology measures, indicating that parcel-level tau largely tracks global tau burden on the effect of FCS.

:::

::: {#suppfig-parcor .column-body-outset}
![](revision_figures/caus_plot.png)

\A A\) Cross-sectional results of the full sample with education in the model and sex as variable of interest. Correlations of the t-maps from pathology and age in the model FC~parcel~ /~ age + AD pathology + sex + motion, when accounting for the variance of nodal affinity averaged across subjects. B) The correlation between the pathology t-map and Gradient 1 persists even though we account for mean nodal affinity, indicating it is not the shared data structure that accounts for the correlation between the two. C) Individual nodal affinity maps correlated with gradient 1 and 3 respectively show that, while individual affinity maps tend to negatively correlate with gradient 1, they do not on average tend to correlate with gradient 3. 
